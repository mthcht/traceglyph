...
function groupedH(dets){var groups={};var so={critical:0,high:1,medium:2,low:3};dets.forEach(function(d){(groups[d.category]=groups[d.category]||[]).push(d)});var sorted=Object.entries(groups).sort(function(a,b){var aMin=Math.min.apply(null,a[1].map(function(x){return so[x.severity]!=null?so[x.severity]:3}));var bMin=Math.min.apply(null,b[1].map(function(x){return so[x.severity]!=null?so[x.severity]:3}));return aMin-bMin});var h='';sorted.forEach(function(pair){var cat=pair[0],items=pair[1];items.sort(function(a,b){return(so[a.severity]!=null?so[a.severity]:3)-(so[b.severity]!=null?so[b.severity]:3)});h+='<div class="grp"><div class="grp-h" onclick="this.classList.toggle(\'collapsed\');this.nextElementSibling.style.display=this.classList.contains(\'collapsed\')?\'none\':\'block\'"><span class="arr">\u25bc</span> '+esc(cat)+' <span class="grp-ct">'+items.length+'</span></div><div>';items.forEach(function(det){h+=cardH(det)});h+='</div></div>'});setTimeout(function(){document.querySelectorAll('.card').forEach(function(c){c.onclick=function(){c.classList.toggle('expanded')}})},0);return h}
